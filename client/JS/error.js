...
const codeExplainBtn = document.getElementById('code-error-btn')
...
async function loadOutputfromOpenAi(inputData) {
    ...
    let convertingPrompt = `find error in the following code and explain how to solve : `

    console.log(convertingPrompt, inputData)
    let response;
    try {
        response = await fetch("http://localhost:5000/error", {
            method: 'POST',
            headers: {
